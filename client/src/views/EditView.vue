...
let user = reactive({
    ...
    regNumber: userStore.user.regNumber,
    nickname: userStore.user.nickname,
    birthday: userStore.user.birthday,
    hobbies: userStore.user.hobbies,
    relStatus: userStore.user.relStatus,
    faveQuote: userStore.user.faveQuote,
    faveCourse: userStore.user.faveCourse,
    faveLecturer: userStore.user.faveLecturer,
    mostStressfulLevel: userStore.user.mostStressfulLevel,
    classCrush: userStore.user.classCrush,
    whatNext: userStore.user.whatNext
})
...
async function submitForm() {
    ...
    try {
        ...
        await deleteFile(userStore.user.imageURL)
        ...
        const res = await fetch(`https://tech-titans-6i23.onrender.com/api/edit-details/${userId}`, {
            method: 'PUT',
            headers: {
...
function validateHobby() {
    hobbyWordCount.value = 30 - user.hobbies.length

    if (user.hobbies.length > 25) {
        error.hobby = true
    } else {
        error.hobby = false
    }

    if (user.hobbies.length > 29) {
    ...
    <div class="bg-gray-200 h-[inherit] min-h-[inherit]">
        ...
        <section class="h-[inherit] min-h-[inherit] overflow-y-scroll p-5 w-full md:w-[32rem] md:mx-auto relative">
            ...
            <div class="card relative">
                ...
                <p class="mt-3">This form has been created to collect important details of the CSC Tech Titans finalists
                    which will be used to generate custom posters showing the profile of each student during the final year
                    week. <br><br>
            ...
            <div class="mt-3 grid gap-3">
                <div class="card form--card">
                    ...
                    <span v-if=error.fullName class="text-xs text-red-500">For the best result please fill in a surname, an optional middle name and a first name. Your middle name will be shortened to an initial.</span>
                ...
                <div class="card form--card">
                    ...
                    <textarea v-model="user.faveQuote" @input="validateQuote" rows="2" placeholder="Your answer" class="resize-none"></textarea>
                </div>
                ...
                <div class="card form--card">
                    ...
                    <textarea v-model="user.whatNext" @input="validateWhatNext" rows="2" placeholder="Your answer" class="resize-none"></textarea>
                </div>
                ...
                <div>
                    <p class="text-xs">By submitting this form, you agree for the data provided to be used and reproduced
                        freely by the Tech Titans.</p>
                </div>
            </div>
