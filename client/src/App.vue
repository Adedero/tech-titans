...
<template>
  ...
  class="flex items-center gap-2 px-5 text-white cursor-context-menu select-none">
    <img src="./assets/images/logo.png" alt="logo" class="w-6">
    <h1
     @click="router.push('/')"
    class="text-2xl">Tech<span class="font-bold">Titans</span></h1>
  </header>

  <Transition name="fade">
    <form
    @submit.prevent="goToAdmin"
    v-if="isOpen" class="w-72 h-60 bg-white absolute z-20 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 shadow-md rounded p-4">
      ...
      <div class="w-full mt-5 flex flex-col gap-3">
        ...
        <input v-model="adminPassword" type="password" placeholder="Enter password" class="border-2 rounded-md p-2 h-10">
      </div>
